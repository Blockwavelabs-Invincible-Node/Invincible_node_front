    ...
    const getData = async() => {
        ...
        if (addresses.length == 0) {
            for (let i = 0 ; i < totalAddresses; i++) {
                ...
                const commission = 5;
                const addr = await stableTokenPoolContract.methods.addressList(i).call();
                setAddresses(addresses => [...addresses, addr]);
                const valiAddr = await stableTokenPoolContract.methods.validatorAddress(addr).call();
                setValiAddresses(valiAddresses => [...valiAddresses, valiAddr]);
                const lend = await stableTokenPoolContract.methods.balanceOf(addr).call();
